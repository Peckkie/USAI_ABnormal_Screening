...
# import data 
dataframe_train = pd.read_csv (r'/home/yupaporn/codes/USAI/traindf1.csv')
dataframe_test = pd.read_csv (r'/home/yupaporn/codes/USAI/validationdf1.csv')
...
prediction_layer2 = layers.Dense(4, activation='softmax',name='Pred_View')(dropout_layer_2)
...
train_generator = train_datagen.flow_from_dataframe(
        ...
        directory = train_dir,
        x_col = 'filename',
        y_col = ['Class_Normal','Class_Abnormal','Views_FP-A','Views_FP-B','Views_FP-C','Views_FP-D'],
...
loss_object = BinaryFocalLoss(gamma=2)#loss************
AbNorweight = 0.75
...
def train_step(model, optimizer, x_train, y_train):
    with tf.GradientTape() as tape:
        ...
        loss1 = loss_object(y_train[0], predictions[0])
        loss2 = loss_object(y_train[1], predictions[1])
...
def test_step(model, x_test, y_test):
    ...
    loss1 = loss_object(y_test[0], predictions[0])
    loss2 = loss_object(y_test[1], predictions[1])
    loss = (AbNorweight*loss1) + ((1-AbNorweight)*loss2)
...
def load_training_data():
    ...
    Y2 = np.column_stack((data[1][2], data[1][3], data[1][4], data[1][5]))
...
train_log_dir = '/media/tohn/SSD/ModelTrainByImage/multi_task/my_logs/' + current_time + '/train'
test_log_dir = '/media/tohn/SSD/ModelTrainByImage/multi_task/my_logs/' + current_time + '/test'
...
model2.save('/media/tohn/SSD/ModelTrainByImage/multi_task/models/Nor_ABnor_b0_R2_multitask_4view_focal.h5')